...
数据库统一使用东八区（UTC+8）：Docker Compose 通过 MySQL `--default-time-zone=+08:00` 设置服务默认时区，应用连接和 SQL 初始化脚本也会显式设置会话时区。API 日期时间统一返回 `YYYY-MM-DD HH:mm:ss`，已有数据库重启后应确认 `SELECT @@global.time_zone, @@session.time_zone;` 均符合部署要求。
...
- 消息接口的日期时间统一以 `YYYY-MM-DD HH:mm:ss` 返回，并且个人消息不会展示发布时间晚于当前东八区时间的消息。
