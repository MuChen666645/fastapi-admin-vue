...
class LogQueryDto(BaseModel):
    ...
    username: str | None = Field(default=None, description="用户名，支持模糊查询",title="用户名")
    status: str | None = Field(
        default=None, pattern="^[01]$", description="登录状态：0失败，1成功",title="登录状态"
    )
    path: str | None = Field(default=None, description="请求路径，支持模糊查询",title="请求路径")
    start_time: datetime | None = Field(default=None, description="查询开始时间",title="查询开始时间")
    end_time: datetime | None = Field(default=None, description="查询结束时间",title="查询结束时间")
...
class LoginLogDto(BaseModel):
    ...
    ip_address: str | None = Field(default=None, description="客户端IP地址", title="客户端IP地址")
    user_agent: str | None = Field(default=None, description="客户端User-Agent", title="客户端User-Agent")
    status: str = Field(description="登录状态：0失败，1成功", title="登录状态")
    message: str | None = Field(default=None, description="登录结果说明", title="登录结果说明")
...
class OperationLogDto(BaseModel):
    ...
    user_id: int | None = Field(default=None, description="操作用户ID", title="操作用户ID")
    username: str | None = Field(default=None, description="操作用户名", title="操作用户名")
...
class ExceptionLogDto(BaseModel):
    ...
    traceback: str | None = Field(default=None, description="异常堆栈", title="异常堆栈")
...
class OnlineSessionDto(BaseModel):
    ...
    ip_address: str | None = Field(default=None, description="登录IP地址", title="登录IP地址")
    user_agent: str | None = Field(default=None, description="客户端User-Agent", title="客户端User-Agent")
    login_time: datetime | None = Field(default=None, description="登录时间", title="登录时间")
...
class OnlineQueryDto(BaseModel):
    ...
    username: str | None = Field(default=None, description="用户名，支持模糊查询", title="用户名")
    ip_address: str | None = Field(default=None, description="登录IP，支持模糊查询", title="登录IP")
